...
namespace PartsControlSystem.Controllers
{
    ...
    public class UpdatePerActivityController : Controller
    {
        ...
        public async Task<IActionResult> UpdateActivity()
        {
            ...
            var activityCards = new List<ActivityCardViewModel>
            {
                new ActivityCardViewModel { ActivityName = "Renewal / Additional Mold",     SectionCounts = GetSectionCounts(x => x.RenewalAdditionalMold) },
                new ActivityCardViewModel { ActivityName = "New Tooling / Localization",     SectionCounts = GetSectionCounts(x => x.NewToolingLocalization) },
                new ActivityCardViewModel { ActivityName = "Transfer Tooling",               SectionCounts = GetSectionCounts(x => x.TransferTooling) },
                new ActivityCardViewModel { ActivityName = "Change Material",                SectionCounts = GetSectionCounts(x => x.ChangeMaterial) },
                new ActivityCardViewModel { ActivityName = "New Model",                      SectionCounts = GetSectionCounts(x => x.NewModel) },
                new ActivityCardViewModel { ActivityName = "Non-Concurrent",                 SectionCounts = GetSectionCounts(x => x.NonConcurrent) },
                new ActivityCardViewModel { ActivityName = "Supplier Change / Localization", SectionCounts = GetSectionCounts(x => x.SupplierChangeLocalization) },
                new ActivityCardViewModel { ActivityName = "Other 4M",                       SectionCounts = GetSectionCounts(x => x.Other4M) }
            };
        ...
        private async Task<IActionResult> HandleToolingRequestOrder(IQueryable<ImportData> query, string process)
        {
            ...
            return PartialView("Partials/MP2/_MP2ToolingRequestOrder", requestOrderList.Select(activity => _updateActivityMapperService.MapRequestOrder(activity)));
        }
        ...
        private async Task<IActionResult> HandleToolingPoIssuance(IQueryable<ImportData> query, string process)
        {
            ...
            return PartialView("Partials/MP2/_MP2ToolingPOIssuance", requestOrderList.Select(activity => _updateActivityMapperService.MapPoIssuance(activity)));
        }
        ...
        private async Task<IActionResult> HandleDFMQCDApproval(IQueryable<ImportData> query, string process)
        {
            ...
            return PartialView("Partials/SQC/_SQCDFM_QCDApproval", requestOrderList.Select(activity => _updateActivityMapperService.MapDfmQcdApproval(activity)));
        }
        ...
        private async Task<IActionResult> HandleToolingFabrication(IQueryable<ImportData> query, string process)
        {
            ...
            return PartialView("Partials/MP2/_MP2ToolingFabrication", requestOrderList.Select(activity => _updateActivityMapperService.MapToolingFabrication(activity)));
        }
        ...
        private async Task<IActionResult> HandleToolingTransfer(IQueryable<ImportData> query, string process)
        {
            ...
            return PartialView("Partials/MP2/_MP2ToolingTransfer", requestOrderList.Select(activity => _updateActivityMapperService.MapToolingTransfer(activity)));
        }
        ...
        private async Task<IActionResult> HandleKatakenSubmission(IQueryable<ImportData> query, string process)
        {
            ...
            return PartialView("Partials/IQC/_IQCKatakenSubmission", requestOrderList.Select(activity => _updateActivityMapperService.MapKatakenSubmission(activity)));
        }
        ...
        private async Task<IActionResult> HandleKatakenFinish(IQueryable<ImportData> query, string process)
        {
            ...
            return PartialView("Partials/IQC/_IQCKatakenFinish", requestOrderList.Select(activity => _updateActivityMapperService.MapKatakenFinish(activity)));
        }
        ...
        private async Task<IActionResult> HandleEvaluation(IQueryable<ImportData> query, string process)
        {
            ...
            return PartialView("Partials/DE/_DEEvaluation", requestOrderList.Select(activity => _updateActivityMapperService.MapEvaluation(activity)));
        }
        ...
        private async Task<IActionResult> HandleSpecialEvaluation(IQueryable<ImportData> query, string process)
        {
            ...
            return PartialView("Partials/QA/_QASpecialEvaluation", requestOrderList.Select(activity => _updateActivityMapperService.MapSpecialEvaluation(activity)));
        }
        ...
        private async Task<IActionResult> HandleTestRun(IQueryable<ImportData> query, string process)
        {
            ...
            return PartialView("Partials/IQC/_IQCTestRun", requestOrderList.Select(activity => _updateActivityMapperService.MapTestRun(activity)));
        }

        // =====================================================================
        // SAVE ACTIONS — importData and activity fetched OUTSIDE if block
        // so WriteTransactionLog always has what it needs
        // =====================================================================
        ...
        public IActionResult SaveMP2ToolingQuotationRequestApprovalUpdate([FromBody] SaveMP2ToolingQuotationRequestApprovalVMDto dto)
        {
            ...
            try
            {
                // ← ADDED: moved outside if block so it's always available
                var importData = _dbContext.ImportDatas
                    .FirstOrDefault(x => x.ControlNo == dto.ControlNumber);
                ...
                // ← ADDED
                WriteTransactionLog(importData, activity, currentProcessValue,
        ...
        // ← ADDED: transaction log writer — inserts one row per save action
